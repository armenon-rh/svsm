...
impl Default for KbsProtocol {
    fn default() -> Self {
        Self {
            backend_type: KbsBackendType::SampleKbs,
        }
    }
}
...
impl AttestationProtocol for KbsProtocol {
    ...
    fn negotiation(
        ...
        let (hash_algo, payload_format) = match self.backend_type {
            KbsBackendType::TrusteeKbs => (HashAlgo::Sha384, PayloadFormat::JwsJson),
            KbsBackendType::SampleKbs => (HashAlgo::Sha512, PayloadFormat::RawBinary),
        };
    ...
    fn attestation(
        ...
        let primary_evidence = match self.backend_type {
            ...
            KbsBackendType::SampleKbs => {
                let AttestationEvidence::Snp {
                    ref report,
                    ref certs_buf,
                } = request.evidence;

                let legacy_evidence = SampleEvidence::Snp {
