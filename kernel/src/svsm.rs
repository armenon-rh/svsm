...
fn load_vtpm_state_poc(kbs_key: &[u8]) -> Option<Vec<u8>>
{
    ...
    log::info!("Found vTPM header: v{}, cipher={}, size={}, secretlen={}",
               version, cipher_id, payload_size, kbs_key.len());

    if cipher_id != 1 || kbs_key.len() != 32 {
        log::error!("Unsupported cipher ID ({}) or invalid kbs key length (expected 32, got {})",
                     cipher_id,
                     kbs_key.len()
                   );
    }

    let payload_offset = 64; // Header size
    ...
    let total_sectors = (total_bytes_to_read + SECTOR_SIZE - 1) / SECTOR_SIZE;
...
fn svsm_init(launch_info: &KernelLaunchInfo) {
    ...
    let kbs_secret = {
        let mut proxy = AttestationDriver::try_from(Tee::Snp)
            .expect("Failed to initialize Attestation Driver");

        let decryption_key = proxy.attest().expect("Attestation failed: could not retrieve key from KBS");
        Some(decryption_key)
    };
    ...
    #[cfg(all(feature = "vtpm", not(test)))]
    {
        ...
        vtpm_init(vtpm_state.as_deref_mut(),
                  kbs_secret.as_deref()
        ).expect("vTPM failed to initialize");
    }
